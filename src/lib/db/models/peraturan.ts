...
export class Peraturan {
  ...
  get nomorPendek() {
    return (
      "No.\u00A0" +
      (this.nomor_text || `${this.nomor}\u00a0Th.\u00a0${this.tahun}`)
    );
  }
...
export async function getListPeraturan({
  ...
  const total = await db
    ...
    .first();
  return { total: (total?.jumlah ?? 0) as number, hasil };
}
...
export async function getPeraturan({ jenis, tahun, nomor }: PeraturanParams) {
  const row = await db
    .prepare(
...
export async function getTanggalTerakhir() {
  const { results } = await db
    .prepare(
...
export async function getListPeraturanByTanggal(tanggal: string) {
  const { results } = await db
    .prepare(
      `SELECT *
       FROM peraturan
...
export async function getFeedListPeraturan(limit = 15) {
  const { results } = await db
    .prepare(
      `SELECT *
       FROM peraturan
